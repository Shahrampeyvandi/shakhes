  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">{{isset($edit) && $edit ? 'ویرایش' : 'ثبت'}} کاربر</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="user--form" method="post" action="{{route('User.Insert')}}" enctype="multipart/form-data">
        @csrf
        @if (isset($edit) && $edit)
        @method('PUT')
        @endif
